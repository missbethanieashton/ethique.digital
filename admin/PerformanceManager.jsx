...
import { TrendingUp, TrendingDown, Users, Eye, MousePointerClick, Share2, Globe, Link2 } from "lucide-react";
...
export default function PerformanceManager() {
  ...
  // Mock data for countries and gender
  const countryData = [
  ...
  // Traffic sources data - aggregate from analytics or use mock data
  const trafficSourcesData = [
    { name: "Instagram", value: 28, percentage: 28 },
    { name: "Google Search", value: 22, percentage: 22 },
    { name: "Facebook", value: 18, percentage: 18 },
    { name: "TikTok", value: 12, percentage: 12 },
    { name: "LinkedIn", value: 8, percentage: 8 },
    { name: "Substack", value: 5, percentage: 5 },
    { name: "Affiliates", value: 4, percentage: 4 },
    { name: "Paid Ads", value: 3, percentage: 3 }
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-light text-white">Performance Analytics</h2>
          <p className="text-sm text-gray-400 mt-1">Track site performance and user engagement</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Gender Distribution */}
        <div className="bg-white/5 border border-white/20 p-6 lg:col-span-2">
          <h3 className="text-lg font-light text-white mb-4">Viewers by Gender</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={genderData}
      ...
      <div className="bg-white/5 border border-white/20 p-6">
        <h3 className="text-lg font-light text-white mb-4">Detailed Traffic Sources</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white/10">
              <tr>
                <th className="px-4 py-3 text-left text-white text-sm font-medium">Source</th>
                <th className="px-4 py-3 text-right text-white text-sm font-medium">Visitors</th>
                <th className="px-4 py-3 text-right text-white text-sm font-medium">Percentage</th>
              </tr>
            </thead>
            <tbody>
              {trafficSourcesData.map((source, index) => (
                <tr key={index} className="border-t border-white/10">
                  <td className="px-4 py-3 text-white">{source.name}</td>
                  <td className="px-4 py-3 text-right text-gray-300">{source.value.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-gray-300">{source.percentage}%</td>
                </tr>
              ))}
            </tbody>
          </table>
